namespace Engine.Input
{
    ...
    public class InputManager : IComponent
    {
        ...
        public bool WorldRectnagleWasJustClicked(Rectangle rec, Camera camera)
        {
            foreach (TouchLocation touchLocation in CurrentTouchCollection)
            {
                Vector2 touchPosition = Vector2.Transform(
                    new Vector2((int)(touchLocation.Position.X / (game.WindowSize.X / game.LogicalSize.X)), (int)(touchLocation.Position.Y / (game.WindowSize.Y / game.LogicalSize.Y))),
